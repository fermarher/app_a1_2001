...
// useStoreVersion — re-render when window.Store fires 'storechange'.
// Components that read window.STATS / window.TOPICS_* / window.FAILED_QUESTIONS
// call this so derived numbers stay live as the user answers questions.
function useStoreVersion() {
  const [, force] = React.useReducer((x) => x + 1, 0);
  React.useEffect(() => {
    const fn = () => force();
    window.addEventListener('storechange', fn);
    return () => window.removeEventListener('storechange', fn);
  }, []);
}

Object.assign(window, { Logo, Wordmark, Icon, Pill, BarTrack, Sparkline, Stat, useMobile, useStoreVersion });
